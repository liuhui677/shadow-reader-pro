...
export class BiquWebParser implements Parser {
  ...
  private async fetchPage(pageURL: string): Promise<void> {
    ...
    //     }
    // });
    this.prevPageURL = `${bookURL}${$('#pb_prev').prop("href")}`;
    this.nextPageURL = `${bookURL}${$('#pb_next').prop("href")}`;
  }
  ...
  async getNextPage(pageSize: number): Promise<string> {
    ...
    this.lastPageSize = pageSize;
    let showText = this.cacheText.slice(
      this.readedCount,
  ...
  async getPrevPage(pageSize: number): Promise<string> {
    ...
    if (
      this.readedCount - pageSize === 0 &&
      this.prevPageURL !== this.indexPageURL
    ) {
