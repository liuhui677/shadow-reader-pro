                ...
                if (bookFuzzyName) {
                    ...
                    window.showQuickPick(Array.from(bookDict.keys()), {matchOnDescription: true}).then(async value => {
                        if (value) {
                            ...
                            window.showQuickPick(Array.from(chapterURLDict.keys()), {matchOnDescription: true}).then( async startChapter => {
                                if(startChapter) {
                                    ...
                                    const url  = await crawler.findRealUrl(<string>chapterURLDict.get(startChapter));
